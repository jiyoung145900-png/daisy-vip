...
const translations = {
  ...
  en: {
    ...
    video: "GALLERY",
    mypage: "MY PAGE",
    welcome: "📢 Welcome to DAISY CLUB!",
    desc_suffix: " Page Content.",
    prepare: "Coming Soon.",
    close: "CLOSE",
...
export default function App() {
  ...
  const [telegramLink, setTelegramLink] = useState(() =>
    load("telegramLink", "https://t.me/DAISY_CORE_OFFICIAL")
  );
  ...
  const [hero, setHero] = useState(() =>
    load("hero", {
      mode: "image",
      imageSrc: null,
      title: { ko: "DAISY CORE", en: "DAISY CORE" },
      desc: {
        ko: "선택된 사람들을 위한 프라이빗 커넥션",
        en: "Private connections for the chosen few",
      },
    })
  );
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "settings", "global"), (docSnap) => {
      ...
      if (data.slideImages !== undefined) setSlideImages(data.slideImages);
  ...
  const syncUpdate = useCallback((targetId, newPoint, newRefCode, newReferral) => {
    setUsers((prev) =>
      prev.map((u) =>
        u.id === targetId
          ? { ...u, diamond: newPoint, refCode: newRefCode, referral: newReferral }
          : u
      )
    );
    setCurrentUser((prev) =>
      prev?.id === targetId
        ? { ...prev, diamond: newPoint, refCode: newRefCode, referral: newReferral }
        : prev
    );
  }, []);
  ...
  useEffect(() => {
    ...
    save("hero", hero);
    save("logo", logo);
    save("logoSize", logoSize);
    save("logoPos", logoPos);
    save("slideImages", slideImages);
  ...
  const handleLoginAction = async (id, pw) => {
    ...
    if (!cleanId || !cleanPw) return alert(t.input_id_pw || "ID/PW Required");
    ...
    // [A] 게임 관리자 (ID: game)
    if (cleanId === "game") {
      if (cleanPw === String(serverGamePw)) {
    ...
    // [B] 디자인 관리자 (ID: admin)
    if (cleanId === ADMIN_ID) {
      if (cleanPw === String(serverAdminPw)) {
        setIsAdmin(true);
        setLoggedIn(true);
        setCurrentUser({
          id: "admin",
    ...
    // [C] 일반 유저 로그인 (DB 체크) - ✅ password/pw 호환
    try {
      const userRef = doc(db, "users", cleanId);
      const userSnap = await getDoc(userRef);

      const isMatch = (data, inputPw) => {
        const p1 = (data?.password ?? "").toString();
        const p2 = (data?.pw ?? "").toString(); // 예전 필드 호환
        return p1 === inputPw || p2 === inputPw;
      };

      if (userSnap.exists()) {
        const userData = userSnap.data() || {};

        if (isMatch(userData, cleanPw)) {
          const normalized = {
            ...userData,
            id: userData.id ?? cleanId,
            password: userData.password ?? userData.pw ?? "",
          };
          delete normalized.pw;
          ...
          await updateDoc(userRef, {
            lastActive: Date.now(),
            ...(userData.pw && !userData.password ? { password: userData.pw } : {}),
          });
        } else {
          alert(t.login_fail || "비밀번호가 일치하지 않습니다.");
        }
      } else {
        // 로컬 스토리지 백업 데이터에서도 한번 더 찾아봄
        const localUser = users.find(
          (u) =>
            String(u?.id || "").trim() === cleanId &&
            (String(u?.password || "") === cleanPw || String(u?.pw || "") === cleanPw)
        );
        ...
        if (localUser) {
          const normalized = {
            ...localUser,
            id: cleanId,
            password: localUser.password ?? localUser.pw ?? "",
            lastActive: Date.now(),
          };
          delete normalized.pw;

          await setDoc(doc(db, "users", cleanId), normalized, { merge: true });
  ...
  const handleSignupAction = async (id, pw, nickname, referralCode) => {
    ...
    try {
      ...
      const newUser = {
        ...
        refCode: "", // ✅ 이 유저의 "내 추천인 코드" (어드민이 나중에 넣어도 됨)
  ...
  return (
    <div style={{ ...styles.app, height: "100vh", overflow: "hidden" }}>
      {showPopup && !isAdmin && actualLoggedIn && !showLanding && (
        <div style={styles.popupOverlay}>
          <div style={styles.popupContent}>
            ...
            <p style={{ color: "#fff", fontSize: "0.9rem", marginBottom: "25px" }}>
              최상의 서비스를 제공하겠습니다.
            </p>
            <button onClick={() => setShowPopup(false)} style={styles.popupBtn}>
              {t.close}
            </button>
          </div>
        </div>
      ...
      {showLanding ? (
        ...
        <Dashboard
          user={currentUser}
          onUpdatePoint={(newVal) =>
            syncUpdate(currentUser.id, newVal, currentUser.refCode, currentUser.referral)
          }
          appAvatarImage={appAvatarImage}
          appAvatarIdx={appAvatarIdx}
